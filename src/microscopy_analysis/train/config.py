...
def load_train_config(path: Path, base_dir: Path | None = None) -> TrainConfig:
    ...
    return TrainConfig(
        ...
        patience=int(raw.get("trainer", {}).get("patience", 30)),
        max_epochs_phase1=int(raw.get("trainer", {}).get("max_epochs_phase1", 120)),
        max_epochs_phase2=int(raw.get("trainer", {}).get("max_epochs_phase2", 60)),
    )
